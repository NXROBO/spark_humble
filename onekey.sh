...
voice_nav(){
	...
	echo -e "${Info}" 
	echo && stty erase ^? && read -p "按回车键（Enter）开始：" 
	print_command "ros2 launch spark_voice ${ASRTYPE}.launch.py camera_type_tel:=${CAMERATYPE}"
	ros2 launch spark_voice ${ASRTYPE}.launch.py camera_type_tel:=${CAMERATYPE}
...
spark_DeepLearn(){
	...
	source ${PROJECTPATH}/install/setup.bash	
	export PYTHONPATH=$PYTHONPATH:/home/spark/models/research
	...
	echo && stty erase ^? && read -p "请选择 1 或 2 ：" chnum
 	case "$chnum" in
		...
		;;
		2)
		print_command "ros2 launch darknet_ros spark_darknet_launch.py camera_type_tel:=${CAMERATYPE}"	
		ros2 launch darknet_ros spark_darknet_launch.py camera_type_tel:=${CAMERATYPE}	
		;;
		*)
...
spark_build_map_3d(){
	...
	if [[ "${SLAMTYPE}" == "rtab_map" ]]; then
		...
		ros2 launch spark_slam depth_slam_teleop.launch slam_methods_tel:=${SLAMTYPE} camera_type_tel:=${CAMERATYPE}
	fi
